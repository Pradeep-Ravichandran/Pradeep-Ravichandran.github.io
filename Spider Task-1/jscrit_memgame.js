...
function flip(tile, val) {
    ...
    document.getElementById(tile).style.background = 'url(orange.jpg)';
    document.getElementById(tile).innerHTML = val;
    if (tilevalues.length == 0) {
        tilevalues.push(val);
        tileids.push(document.getElementById(tile).id);
        tries++;
        document.getElementById("nooftries").innerHTML = "MOVES : " + tries;
    } else if (tilevalues.length == 1) {
        tilevalues.push(val);
        tileids.push(document.getElementById(tile).id);

        if ((tilevalues[0] == tilevalues[1]) && (tileids[0] != tileids[1])) {
            matches += 2;
